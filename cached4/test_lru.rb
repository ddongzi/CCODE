...
# 测试插入功能
def test_put(socket)
  response = send_command(socket, "PUT key1 value1")
  puts "PUT: #{response}"

  response = send_command(socket, "PUT key2 value2")
  puts "PUT: #{response}"

  response = send_command(socket, "PUT key3 value3")
  puts "PUT: #{response}"
end
...
def test_get(socket)
  ...
  response = send_command(socket, "GET key3")
  puts "GET key3: #{response}"

  response = send_command(socket, "GET key4")  # 不存在的键
  puts "GET key4: #{response}"
end

# 测试删除功能
def test_delete(socket)
  response = send_command(socket, "DELETE key1")
  puts "DELETE key1: #{response}"

  response = send_command(socket, "DELETE key2")
  puts "DELETE key2: #{response}"

  response = send_command(socket, "DELETE key3")
  puts "DELETE key3: #{response}"

  response = send_command(socket, "DELETE key4")  # 不存在的键
  puts "DELETE key4: #{response}"
end

# 测试QUIT命令
def test_quit(socket)
  response = send_command(socket, "QUIT")
  puts "QUIT: #{response}"
end

# 测试SELECT命令
def test_select(socket)
    response = send_command(socket, "SELECT")
...
def test_lru(socket)
  # 设置缓存大小为5
  (1..6).each do |i|
    send_command(socket, "PUT key#{i} value#{i}")
    sleep(1)  # 等待1秒
  end
